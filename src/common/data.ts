...
export async function readTestCasesFromYamlAsync<T extends TestCase<any, any>>(fileName: string,
                                                                               prefix?: string): Promise<T[]> {
...
export function readCsvData(fileName: string, prefix?: string, delimiter: string = ",", autoCast: boolean = true): any[] {
    return loadCsv(readDataFile(fileName, prefix), {
        columns: true, skip_empty_lines: true, delimiter: delimiter, ltrim: true, cast: autoCast, castDate: autoCast
    });
}
